...
async function fetchGenres() {
    const responсe = await axios.get(`https://api.themoviedb.org/3/genre/movie/list${KEY}&language=en-US`);
    const genres = await responсe;
...
async function fetchMoviesByQuery(movie, page) {
    const responсe = await axios.get(`https://api.themoviedb.org/3/search/movie${KEY}${QUERY}&query=${movie}`);
    const movies = await responсe;
    return movies.data;
}
